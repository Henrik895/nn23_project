...
def generate_image(sample,
    ...
    placement = cv2.resize(placement, [int(
        placement.shape[1]*0.3*scale), int(placement.shape[0]*0.3*scale)])
    if location_of_placement == 0:
        offset0 = randint(50, 250)
        offset1 = randint(50, 250)
        # Add text in middle of placement
        if randint(0, 1) == 0:
            first_letter = get_random(
                ['e', 'f', 'l', 'm', 'n'][randint(0, 4)], sample_extras)
            second_letter = get_random(
                ['e', 'f'][randint(0, 1)], sample_extras)
            first_letter = resize_by_scale(first_letter, scale)
            second_letter = resize_by_scale(second_letter, scale)

            placement_mid = (
                int(placement.shape[0]/2), int(placement.shape[1]/2))

            placement = place_symbol(placement, first_letter, placement_mid[0]-int(
                first_letter.shape[0]/2), placement_mid[1]-int(first_letter.shape[1])-10)
            placement = place_symbol(
                placement, second_letter, placement_mid[0]-int(second_letter.shape[0]/2), placement_mid[1]+10)

        canvas = place_symbol(canvas, placement, offset0,
                              canvas.shape[1]-placement.shape[1]-offset1)
        ...
        point_up = np.arange(placement.shape[0])[
            placement[:, 50] < excess_str][1]
        point_down = np.arange(placement.shape[0])[
            placement[:, 50] < excess_str][-2]
        point_left = np.arange(placement.shape[1])[
            placement[-50, :] < excess_str][1]
        point_right = np.arange(placement.shape[1])[
            placement[-50, :] < excess_str][-2]
        # Add location numbers to placement symbols
        # Generate the grid number
        grid1 = randint(0, 99)
        canvas = add_grid_number(canvas, point_down, offset0, offset1,
                                 grid1, placement, sample_extras, scale=0.75*scale)
        canvas = add_grid_number(canvas, point_up, offset0, offset1,
                                 (grid1+1) % 100, placement, sample_extras, scale=0.75*scale)
        grid2 = randint(0, 99)
        canvas = add_grid_number2(
            canvas, point_left, offset0, offset1, grid2, placement, sample_extras, scale=0.75*scale)
        canvas = add_grid_number2(canvas, point_right, offset0, offset1,
                                  (grid2+1) % 100, placement, sample_extras, scale=0.75*scale)
        length = point_down-point_up
        point1 = offset0+point_down + \
            ((canvas.shape[0]-offset0-point_down)//length-1)*length
        point2 = (canvas.shape[1]-offset1 -
                  (placement.shape[1]-point_left)) % length+length
        ...
        grid1 = (
            grid1 - ((canvas.shape[0]-offset0-point_down)//length-1)) % 100
        canvas = add_grid_number(canvas, point1, 0, canvas.shape[1]-point2-int(
            placement2.shape[1]/2), grid1, placement2, sample_extras)
        grid2 = (
            grid2 - int((canvas.shape[1]-offset1-(placement.shape[1]-point_left+length)-point2)/length)) % 100
        canvas = add_grid_number2(canvas, -point2, point1-int(
            placement2.shape[0]/2), canvas.shape[1], grid2, placement2, sample_extras)
        location_placement.append(
            ((point1_1, point2_1), (point1_1+placement2.shape[0], point2_1+placement2.shape[1])))
    ...
    for task in range(randint(3, 6)):  # Nr of symbols on image
        ...
        if random.uniform(0, 1) > real_symbols_ratio:
            ...
            if(label in sample_real_Clean):
                ...
                imgClean, imgDirty = real_symbol_utils.get_random_pair(
                    label, sample_real_Clean, sample_real)
                from_real_film = True
        ...
        if not from_real_film:
            ...
            if label in ['screen', 'cover', 'guard']:
                if random.uniform(0, 1) > 0.5:
                    ...
                    img, rotation = augment(
                        img, apply_rotation=True, apply_transformation=True, apply_boldness=True)
            else:
                # Augment the image
                img, rotation = augment(
                    img, apply_rotation=True, apply_transformation=True, apply_boldness=True)
            ...
            rotation = 0
            img, rotation, point_1, point_2 = real_symbol_utils.add_real_symbol(
                imgClean, imgDirty, scale)
        ...
        try:
            point1, point2 = get_points(
                dim, img, locations, locations_units, location_placement)
        except:
            continue

        if unit:
            locations_units.append(
                ((point1, point2), (point1+img.shape[0], point2+img.shape[1])))
        ...
        locations.append(((point1+point_1[0], point2+point_1[1]),
                         (point1+img.shape[0]+point_2[0], point2+img.shape[1]+point_2[1])))
        ...
        if label in ['attack', 'counterattack', 'advance_to_contact']:
            if random.uniform(0, 1) > 0.4:
                try:
                    canvas = draw_line(canvas, point1, point2,
                                       rotations[-1], 1, img)
                    canvas = draw_line(canvas, point1, point2,
                                       rotations[-1], -1, img)
                except:
        ...
        if label not in ['screen', 'cover', 'guard']:
            if random.uniform(0, 1) > 0.2:
                ...
                unit_symbol, unit_lab = generate_unit(sample_units, label,
                                                      ...
                                                      resizable_vertical, unit_sizes)
                unit_symbol = cv2.resize(
                    unit_symbol, [int(unit_symbol.shape[1]*scale), int(unit_symbol.shape[0]*scale)])
                ...
                center = [int(point1+img.shape[0]/2),
                          int(point2+img.shape[1]/2)]
                point2_1, point1_1 = point_location(
                    rotations[-1], img.shape, center, unit_symbol.shape)
                ...
                if ((point1_1+unit_symbol.shape[0] < dim[0]) and (point2_1+unit_symbol.shape[1] < dim[1]) and (point1_1 >= 0) and (point2_1 >= 0)):
                    ...
                    if not is_overlap:
                        canvas = place_symbol(
                            canvas, unit_symbol, point1_1, point2_1)
                        locations_units.append(
                            ((point1_1, point2_1), (point1_1+unit_symbol.shape[0], point2_1+unit_symbol.shape[1])))
                        labels_units.append(unit_lab)
    ...
    for i in range(randint(1, 4)):
        ...
        try:
            point1, point2 = get_points(
                dim, mortar_img, locations, locations_units, location_placement)
        except:
            continue
...
def main(
    ...
    save_unit_labels_dir='unit_labels/train',
    ...
    if(real_symbols_ratio != 0):
        for dir in os.listdir(real_symbols_dir):
            sample_real = real_symbol_utils.read_into_dic(
                f'{real_symbols_dir}/{dir}', symbols_regex, sample_real)

        for dir in os.listdir(real_symbols_clean_dir):
            sample_real_Clean = real_symbol_utils.read_into_dic(
                f'{real_symbols_clean_dir}/{dir}', symbols_regex, sample_real_Clean)
    ...
    if not os.path.exists(save_unit_labels_dir):
        os.makedirs(save_unit_labels_dir)

    # references for real background generation
    backgroundImageList = background_utils.ProcessBackgrounds(
        real_backgrounds_dir)

    realBackgroundSampleCount = round(examples_nr*real_backgrounds_ratio)
    realBackgroundSample = random.sample(
        range(examples_nr), realBackgroundSampleCount)
        ...
        while not_successful:
            try:
                ...
                if(i in realBackgroundSample):
                    canvas, boundingBoxesToRemove, background_dim = random.choice(
                        backgroundImageList)
                    canvas = canvas.copy()
                ...
                img = cv2.resize(img.astype('float32'), (int(
                    save_dim[1]), int(save_dim[0]))).astype('int16')
                if save_as_square:
                ...
                labels2 = list(
                    map(lambda label: get_labels(label, labels_to_nr), labels))
                ...
                with open(f'{save_labels_dir}/img{i}.txt', 'w') as f:
                    for k, lab in enumerate(labels2):
                        if (k != len(labels2)-1):
                            f.write(
                                f'{lab} {locations2[k,0]} {locations2[k,1]} {locations2[k,2]} {locations2[k,3]}\n')
                        else:
                            f.write(
                                f'{lab} {locations2[k,0]} {locations2[k,1]} {locations2[k,2]} {locations2[k,3]}')
                ...
                with open(f'{save_unit_labels_dir}/img{i}.txt', 'w') as f:
                    unit_locations = get_locations(
                        loc_units, dim[1], dim[0], offset)
                    for k, unit_label in enumerate(lab_units):
                        # label_id = (manuever_units + support_units).index(unit_label)
                        if (k != len(labels2)-1):
                            f.write(
                                f'{unit_label} {unit_locations[k,0]} {unit_locations[k,1]} {unit_locations[k,2]} {unit_locations[k,3]}\n')
                        else:
                            f.write(
                                f'{unit_label} {unit_locations[k,0]} {unit_locations[k,1]} {unit_locations[k,2]} {unit_locations[k,3]}')
...
def parse_opt():
    ...
    parser.add_argument('--dim_h', type=int, default=3468,
                        help='The dimension which is being used during generating, should be same in which the symbols samples are taken')
    parser.add_argument('--dim_w', type=int, default=4624,
                        help='The dimension which is being used during generating, should be same in which the symbols samples are taken')
    parser.add_argument('--save_dim_h', type=int, default=578,
                        help='Dimesion in which the generated images are saved')
    parser.add_argument('--save_dim_w', type=int, default=770,
                        help='Dimesion in which the generated images are saved')
    parser.add_argument('--save_as_square', type=bool, default=False,
                        help='If the saved image hieght and width are equal. Uses save_dim[1] as dimension for both')
    parser.add_argument('--save_as_inverse', type=bool, default=False,
                        help='Switches black and white pixels when saving')
    parser.add_argument('--vertical_ratio', type=float, default=0.0,
                        help='Probabilty that generated image is vertical / will switch dim_h and dim_w')
    parser.add_argument('--examples_nr', type=int, default=1,
                        help='Number of images to generate')
    parser.add_argument('--symbols_dir', type=str, default='data/symbols',
                        help='Directory in which the sample of tactical tasks are')
    parser.add_argument('--real_symbols_dir', type=str, default='data/real_symbols',
                        help='Directory in which the sample of tactical tasks cut from real films are')
    parser.add_argument('--real_symbols_clean_dir', type=str, default='data/real_clean_symbols',
                        help='Directory in which the sample of tactical tasks cut from real films are')
    parser.add_argument('--unit_symbols_dir', type=str, default='data/unit_symbols',
                        help='Directory in which the sample of unit symbols are')
    parser.add_argument('--extras_dir', type=str, default='data/extras',
                        help='Directory in which the sample of extras is')
    parser.add_argument('--save_images_dir', type=str,
                        default=f'images/{subfolder}', help='Directory where to store generated images')
    parser.add_argument('--save_labels_dir', type=str,
                        default=f'labels/{subfolder}', help='Directory where to store labels')
    parser.add_argument('--save_unit_labels_dir', type=str,
                        default=f'unit_labels/{subfolder}', help='Directory where to store unit labels')
    parser.add_argument('--save_rotations_dir', type=str,
                        default=f'rotations/{subfolder}', help='Directory where to store rotations')
    parser.add_argument('--real_backgrounds_ratio', type=float,
                        default=0.0, help='Ratio of data with real backgrounds')
    parser.add_argument('--real_backgrounds_dir', type=str, default="data/real_backgrounds",
                        help='Directory in which the real data backgrounds are')
    parser.add_argument('--real_symbols_ratio', type=float,
                        default=0.0, help="Ratio of real symbols cut from film")
    parser.add_argument('--real_symbols_in_real_backgrounds', action='store_true',
                        help="use real symbols in real backgrounds while generating real backgrounds data")
